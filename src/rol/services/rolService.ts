...
export class RolService {
    ...
    public static async getAllRoles(): Promise<Rol[]> {
        try {
    ...
    public static async getRolByTitle(title: string): Promise<Rol | null> {
        try {
            return await RolRepository.findByTitle(title);
        } catch (error: any) {
            throw new Error(`Error al encontrar rol: ${error.message}`);
        }
    }
